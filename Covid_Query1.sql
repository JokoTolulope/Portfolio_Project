...
SELECT Death.continent, Death.location, Death.date, Death.population, Vacc.new_vaccinations, Vacc.total_vaccinations,
AVG(CAST(Vacc.[total_vaccinations] AS Integer)) OVER (PARTITION BY Death.location ORDER BY Death.location) AS Avg_VaccinatedPeople_perlocation
FROM [dbo].[CovidDeaths$] AS Death
JOIN [dbo].[CovidVaccinations$] AS Vacc
     ON Death.location = Vacc.location
	 AND Death.date = Vacc.date
